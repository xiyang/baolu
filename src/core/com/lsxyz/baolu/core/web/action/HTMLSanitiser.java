...
public class HTMLSanitiser {
	...
	private static final Set<String> VALID_ATTRIBUTE_NAMES=new HashSet<String>(Arrays.asList(new String[] {
		"id","class","href","target","title", "src"
	}));
		...
		for (Tag tag : tags) {
			if (processTag(tag, outputDocument)) {
				...
				if (!stripInvalidElements)
					continue; // element will be encoded along with
								// surrounding text
   ...
   public static List<String> getAttributeValue(String input, String tagName, String tagProperty){
		
		List<String> tagPropertyValues = new ArrayList<String>();
		Source source = new Source(input);
		List<Tag> tags = source.getAllTags();
		for (Tag tag : tags) {
			if(StringUtils.equalsIgnoreCase(tagName, tag.getName())){
				StartTag startTag = tag.getElement().getStartTag();
				
				for (Attribute attribute : startTag.getAttributes()) {
					if( StringUtils.equalsIgnoreCase(tagProperty, attribute.getKey())){
						tagPropertyValues.add(attribute.getValue());
					}
				}
			}
		}
    ...
    public static List<String> getPictureAttributeValueByNoExist(String input, String tagName, String tagProperty){
		...
		for (Tag tag : tags) {
			if(StringUtils.equalsIgnoreCase(tagName, tag.getName())){
				StartTag startTag = tag.getElement().getStartTag();
				if(startTag.getAttributes().get("height") != null && StringUtils.equals(startTag.getAttributes().get("height").getValue(), "0")
						  || startTag.getAttributes().get("width") != null && StringUtils.equals(startTag.getAttributes().get("width").getValue(), "0")){
					for (Attribute attribute : startTag.getAttributes()) {
						if( StringUtils.equalsIgnoreCase(tagProperty, attribute.getKey())){
							tagPropertyValues.add(attribute.getValue());
						}
   ...
   public static List<String> getPictureAttributeValueByExist(String input, String tagName, String tagProperty){
...
//       str = str.replaceAll("[(/>)<]", "");  
	...
	public static void main(String[] args) throws Exception {
		...
		String str = "<html>if(45>3)haiwei</html>";;
		String html = "<html>List:wo kao <br> <p>xxx xxx</p><img src=\"/abgent/xx/jj.jpg\"/><li>A</li><li>B<li>C<B>this is b!</B><table style='width=10px'><tr><td><img src=\"dddd.gif\"/>3434 34342 13132</td><tr></table></html>";
		System.out.println(splitAndFilterHTMLString(html));
		
//		displayStripInvalidMarkup("List:wo kao <br> <p>xxx xxx</p><img src=\"/abgent/xx/jj.jpg\"/><li>A</li><li>B<li>C<B>this is b!</B><table><tr><td><img src=\"dddd.gif\"/>3434 34342 13132</td><tr></table>");
	}

	private static void displayStripInvalidMarkup(String input) {
		for(String value : HTMLSanitiser.getAttributeValue(input, "img", "src")){
